...
double ***calculoProximosElementos(double ***z, int m, int n, double dx, double dy, FILE *arquivo){
	int i, j, k;
	for (k = 1; k < tf; ++k)
	{
		for (i = 1; i < n-1; ++i)
			...
			for (j = 1; j < m-1; ++j)
			{
				...
				if (k == tf-1)
				{
					fprintf(arquivo, "%lf\t%lf\t%lf\t",i*dx, j*dy ,z[i][j][(k+1)%3]);
					fprintf(arquivo,"\n");
...
int main(){
	...
	z = calculoProximosElementos(z, m, n, dx, dy, arquivo);
	//Fechando o arquivo
	fclose(arquivo);
